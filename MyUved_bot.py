...
async def list_notifications(message: types.Message, state: FSMContext):
    ...
    sorted_notifs = sorted(notifications.items(), key=lambda x: x[1]['time'] if not x[1].get('repeat_type') else 0)
    tz = pytz.timezone(config.get('timezone', 'Europe/Moscow'))
    ...
    for notif_id, notif in sorted_notifs:
        ...
        if repeat_type == 'every_day':
            text = (
                ...
                f"📝 **Текст:** {notif['text']}\n"
                f"📅 **Тип:** Ежедневное\n"
                f"⏰ **Время:** {notif.get('repeat_hour', 0):02d}:{notif.get('repeat_minute', 0):02d}\n"
                f"📊 **Статус:** АКТИВНО"
            )
        elif repeat_type == 'weekdays':
