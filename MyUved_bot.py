...
def parse_date(date_str: str) -> Optional[datetime]:
    ...
    if match:
        ...
        try:
            ...
            return tz.localize(datetime(int(year), int(month), int(day), int(hour), int(minute)))
        except:
            return None
    ...
    if match:
        ...
        try:
            tz = pytz.timezone(config.get('timezone', 'Europe/Moscow'))
            return tz.localize(datetime(int(year), int(month), int(day), now.hour, now.minute))
        except:
            return None
    ...
    if match:
        ...
        try:
            tz = pytz.timezone(config.get('timezone', 'Europe/Moscow'))
            result = tz.localize(datetime(int(year), int(month), int(day), now.hour, now.minute))
            if result < now:
                result = tz.localize(datetime(int(year) + 1, int(month), int(day), now.hour, now.minute))
            return result
        except:
            return None
...
def parse_weekdays(weekdays_str: str) -> List[int]:
    ...
    weekdays_str = weekdays_str.upper().replace(' ', '')
    days = []
    
    # Разделяем по запятым или пробелам
    parts = re.split(r'[, ]+', weekdays_str)
...
def get_next_weekday(target_weekdays: List[int], time_str: str = None) -> Optional[datetime]:
...
async def get_time_type(callback: types.CallbackQuery, state: FSMContext):
    ...
    if time_type == 'hours':
        ...
        await bot.send_message(
            ...
            "📅 **Введите дни недели**\n\n"
            "📝 **Поддерживаемые форматы:**\n"
            "• `ПН, ВТ, СР` - через запятую\n"
            "• `ПН ВТ СР` - через пробел\n"
...
@dp.message_handler(state=NotificationStates.waiting_for_weekdays)
async def set_weekdays(message: types.Message, state: FSMContext):
    try:
        weekdays_list = parse_weekdays(message.text)
        if not weekdays_list:
            await message.reply(
                "❌ **Ошибка!** Не удалось распознать дни недели.\n"
                "📝 Примеры: `ПН, ВТ, СР` или `Понедельник Вторник Среда`",
                parse_mode='Markdown'
            )
            return
        
        await state.update_data(weekdays_list=weekdays_list)
...
async def set_weekday_time(message: types.Message, state: FSMContext):
    try:
        ...
        for i in range(1, 8):
            next_date = now + timedelta(days=i)
            if next_date.weekday() in weekdays_list:
                first_time = tz.localize(datetime(next_date.year, next_date.month, next_date.day, hour, minute))
                break
        ...
        for d in sorted(weekdays_list):
            days_names.append(weekday_names[d])
...
async def list_notifications(message: types.Message, state: FSMContext):
    ...
    sorted_notifs = sorted(notifications.items(), key=lambda x: x[1]['time'] if not x[1].get('repeat_type') else x[1].get('repeat_hour', 0))
    tz = pytz.timezone(config.get('timezone', 'Europe/Moscow'))
    ...
    for notif_id, notif in sorted_notifs:
        ...
        if repeat_type == 'every_day':
            status = "🔄 ЕЖЕДНЕВНОЕ"
            time_str = f"{notif.get('repeat_hour', 0):02d}:{notif.get('repeat_minute', 0):02d}"
            text = (
            ...
            weekday_names = ['ПН', 'ВТ', 'СР', 'ЧТ', 'ПТ', 'СБ', 'ВС']
            days = [weekday_names[d] for d in notif.get('weekdays_list', [])]
            status = "📆 ПО ДНЯМ НЕДЕЛИ"
            time_str = f"{notif.get('repeat_hour', 0):02d}:{notif.get('repeat_minute', 0):02d}"
            text = (
                f"📆 **Уведомление #{notif.get('num', notif_id)}**\n"
                f"📝 **Текст:** {notif['text']}\n"
...
async def handle_snooze(callback: types.CallbackQuery):
    ...
    if notif_id in notifications:
        # Проверяем, повторяющееся ли уведомление
        if notifications[notif_id].get('repeat_type'):
            # Для повторяющихся уведомлений не меняем время
