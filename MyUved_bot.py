...
# Декоратор для автоматического сброса состояния по таймауту
def timeout_state(timeout_seconds=180):
    """Декоратор для автоматического сброса состояния через заданное время"""
    def decorator(handler):
        async def wrapper(message: types.Message, state: FSMContext, *args, **kwargs):
            # Устанавливаем таймаут
            await state.update_data(last_activity=datetime.now().isoformat())
            
            # Запускаем таймер на сброс
            async def reset_after_timeout():
                    ...
                    if last_activity:
                        ...
                        if (datetime.now() - last_time).total_seconds() >= timeout_seconds:
                            ...
                            await message.reply(
                                ...
                                "Операция отменена. Начните заново с команды /start",
                                parse_mode='Markdown'
                            )
            
            asyncio.create_task(reset_after_timeout())
...
async def list_notifications(message: types.Message, state: FSMContext):
    # Сбрасываем состояние перед выводом списка
    await state.finish()
...
async def toggle_notifications(callback: types.CallbackQuery):
    ...
    await bot.send_message(callback.from_user.id, f"✅ **Уведомления {status}!**", parse_mode='Markdown')
    await settings_menu(callback.message, callback.from_user.id)
    await callback.answer()
...
# Обработчик для всех остальных сообщений (сброс "залипших" состояний)
@dp.message_handler(state='*', content_types=types.ContentTypes.ANY)
async def handle_stuck_state(message: types.Message, state: FSMContext):
    current_state = await state.get_state()
    if current_state is not None:
        ...
        if last_activity:
            last_time = datetime.fromisoformat(last_activity)
            if (datetime.now() - last_time).total_seconds() > 180:  # 3 минуты
                await state.finish()
                await message.reply(
                    "⏰ **Время ожидания истекло!**\n"
                    "Операция отменена. Начните заново с команды /start",
        ...
        # Если пользователь ввел команду /start или другую команду меню
        if message.text in ["➕ Добавить уведомление", "📋 Список уведомлений", "⚙️ Настройки", "💾 Создать бэкап"]:
            await state.finish()
            await cmd_start(message, state)
