    ...
    while True:
        if notifications_enabled:
            ...
            for notif_id, notif in list(notifications.items()):
                ...
                # Конвертируем время уведомления в текущий часовой пояс
                ...
                if now >= notify_time and not notif.get('notified', False):
                    ...
                    notifications[notif_id]['message_id'] = sent_message.message_id
                    save_data()
...
async def ask_for_code(callback: types.CallbackQuery):
    ...
    # Запланировать удаление сообщения через 3 минуты
...
async def ask_for_direct_token(callback: types.CallbackQuery):
    ...
    async def delete_message():
        ...
        try:
            await bot.delete_message(callback.from_user.id, msg.message_id)
            await bot.send_message(callback.from_user.id, "⏰ Время вышло! Начните заново с /start")
        except:
            pass
...
async def add_notification_start(message: types.Message, state: FSMContext):
    ...
    async def delete_message():
        ...
        try:
            ...
            await bot.send_message(message.chat.id, "⏰ Время вышло! Начните заново с /start")
        except:
            pass
...
async def cancel_tz(callback: types.CallbackQuery):
    await settings_menu(callback.message, callback.from_user.id)
    await callback.answer()
...
async def select_backup(callback: types.CallbackQuery):
